...
import { FlatList, ListView, View, Platform, Text } from 'react-native';
...
class CalendarList extends Component {
	...
	constructor(props) {
		...
		this.futureScrollRange =
			props.futureScrollRange === undefined
				? 50
				: props.futureScrollRange;
		...
		for (
			let i = 0;
			i <= this.pastScrollRange + this.futureScrollRange;
			i++
		) {
			const text = date
				.clone()
				.addMonths(i - this.pastScrollRange)
				.toString('MMM yyyy');
			rows.push(text);
			texts.push(text);
		}
	...
	renderCalendar(row) {
		if (row.getTime) {
			...
			return (
				<View
					style={[{ height: calendarHeight }, this.style.placeholder]}
				>
	...
	onScroll(event) {
		...
		if (Math.abs(yOffset - this.lastScrollPosition) > calendarHeight) {
			...
			for (let i = 0; i < rows.length; i++) {
				...
				const rowShouldBeRendered =
					Math.abs(rowStart - yOffset) < calendarHeight * 2;
	...
	render() {
		...
		return (
			<FlatList
				...
				initialListSize={
					this.pastScrollRange * this.futureScrollRange + 1
				}
